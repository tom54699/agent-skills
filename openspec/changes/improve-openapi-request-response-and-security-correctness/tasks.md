...
- [x] 1.1 擴充 `skills/.curated/laravel-api-docs/src/InferCandidates/FormRequestParser.php`，支援 array-style rules，並保留目前會漏掉的欄位。
- [x] 1.2 補 `regex`、`same`、`Password::min()->letters()->numbers()` 等常見 Laravel 規則的解析與映射。

## 2. Success Response Parsing

- [x] 2.1 擴充 `skills/.curated/laravel-api-docs/src/InferCandidates/ControllerParser.php`，讓 `apiResponse()` 成功 payload 可抽出 data expression。
- [x] 2.2 擴充 `skills/.curated/laravel-api-docs/src/OpenApiGenerator/OpenApiGenerator.php`，優先用 `apiResponse()` 成功 payload 生成 example，避免 path heuristic 誤判。
