...
- [x] 1.1 擴充 `skills/laravel-api-docs/src/InferCandidates/FormRequestParser.php`，補齊高頻 Laravel request rules 的 normalization。
- [x] 1.2 將 Password builder 拆成 capability，而不是視為單一規則字串。
- [x] 1.3 為部分可映射與不可可靠映射的規則保留 unresolved metadata。

## 2. Nested Schema Generation

- [x] 2.1 擴充 `skills/laravel-api-docs/src/OpenApiGenerator/OpenApiGenerator.php`，將 dotted fields 轉成 nested object schema。
...
- [x] 3.1 更新 `skills/laravel-api-docs/SKILL.md`，說明 request rule normalization 與 unresolved rule 行為。
